...
type
  ...
  TVectorEditor = class(TForm)
    ...
    procedure PaletteGridClick(Sender: TObject);
...
begin
  index := 0;
  rate := floor($ffffff / (PaletteGrid.RowCount * PaletteGrid.ColCount));
  ...
  for col := 0 to PaletteGrid.ColCount do begin
    ...
    for row := 0  to PaletteGrid.RowCount do begin
      ...
      PaletteColors[col, row] := $000000 + rate * index;
      index += 1;
    end;
  end;
end;
...
procedure TVectorEditor.PaletteGridClick(Sender: TObject);
begin
